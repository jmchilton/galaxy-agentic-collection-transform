...
- `lib/galaxy/util/rules_dsl_spec.yml` - Test specification
- `lib/galaxy/managers/collections.py` - Collection building from rules
- GitHub PR #5819 - Original implementation
...
1. **Always start with metadata columns** - Use `add_column_metadata` to get identifiers/indices
2. **Build up columns, then map** - Rules create data, mapping interprets it
3. **Column indices are 0-based** - First added column is 0
4. **Order matters in list_identifiers** - First column is outermost nesting level
5. **Use regex for complex extraction** - Can split identifiers, rename values
6. **Filter before restructuring** - Remove unwanted elements early
7. **Clean up with remove_columns** - Remove intermediate columns not needed in mapping
8. **Tags can drive grouping** - Use `add_column_group_tag_value` for experimental designs
