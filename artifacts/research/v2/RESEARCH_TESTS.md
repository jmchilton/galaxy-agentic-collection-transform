...
#### Creating List:Paired with Full Control

```python
response = dataset_collection_populator.upload_collection(
    history_id,
    "list:paired",
    elements=[
        {
...
#### Additional Collection Creation Helpers

```python
# Create nested collection from existing
hdca = dataset_collection_populator.create_nested_collection(
    history_id=history_id,
    collection_type="list:list:paired",
    collection=[existing_hdca_id]  # Wrap existing collection
)

# Create list from pairs
pair1_id = create_pair_in_history(history_id, contents=["1", "2"])
pair2_id = create_pair_in_history(history_id, contents=["3", "4"])
hdca = dataset_collection_populator.create_list_from_pairs(
    history_id,
    [pair1_id, pair2_id]
)
```
...
#### Subcollection Mapping Patterns Reference

```
list:paired + map_over_type: "paired" → maps over each paired within the list
list:list + map_over_type: "list" → maps over inner lists
list:list:paired + map_over_type: "list:paired" → maps over inner list:paired
```
...
### 7. Asynchronous Operations & Best Practices
...
**Why This Matters:** Datasets may still be uploading or processing. Running operations on incomplete datasets causes failures or incorrect results.
...
**Why This Matters:** Jobs execute asynchronously. Accessing output content before job completion returns incomplete or empty data.
...
**Why This Matters:** Map-over creates implicit collections. The collection structure may be created before all elements are populated. Always verify `populated_state == "ok"` before using collection contents.
...
**Lesson:** Cannot map over collections with different structures (paired vs list) in linked mode.

---

#### Edge Case: Invalid Index in Extract Dataset

```python
inputs = {
    "data_collection": {"src": "hdca", "id": collection_id},
    "which": {"which_dataset": "by_index", "index": 100}  # Invalid
}
# Returns 400 error:
# err_msg: "Dataset collection has no element_index with key 100."
```

---

#### Edge Case: Invalid Paired Identifiers

```python
# Using wrong identifiers for paired_identifier mapping
rules = {
    "mapping": [
        {"type": "paired_identifier", "columns": [1]}  # Column 1 has "floorward"/"reverb"
    ]
}
# Returns 400 error:
# "Unknown indicator of paired status encountered (floorward)"
# Valid values: forward/reverse, f/r, R1/R2, 1/2
```

---

### 9. Response Structure Reference
...
response = {
    "outputs": [
        {"id": "...", "name": "...", "hid": 1}  # Dataset outputs
    ],
    "output_collections": [
        {"id": "...", "name": "...", "hid": 2, "collection_type": "list"}  # Direct collection outputs
    ],
    "implicit_collections": [
        {"id": "...", "hid": 3, "collection_type": "list:paired"}  # Collections from mapping
    ],
    "jobs": [
...
**Key Distinction:**
- `outputs`: Individual datasets created directly by tool
- `output_collections`: Collections explicitly created by tool (e.g., __BUILD_LIST__)
- `implicit_collections`: Collections created by map-over operations
...
```python
hdca = dataset_populator.get_history_collection_details(history_id, hid=collection_hid)
# Returns:
{
    "id": "...",
                ...
                "elements": [
                    {
                        ...
                        "element_type": "hda",
                        "object": {"id": "...", "state": "ok", "file_ext": "txt"}
                    }
                ]
            }
        }
...
#### Collection Type Transitions Reference

| Operation | Input Type | Output Type |
|-----------|------------|-------------|
| Unzip | paired | 2 datasets |
| Unzip (mapped) | list:paired | 2 lists |
| Zip | 2 datasets | paired |
| Zip (mapped) | 2 lists | list:paired |
| Flatten | list:paired | list |
| Nest | list | list:list |
| Filter | list:* | list:* (same type) |
| Apply Rules | any | configurable |

---

### 10. Testing Patterns Summary
...
assert hdca["elements"][0]["element_identifier"] == "sample1"

# Check element type
assert hdca["elements"][0]["element_type"] == "dataset_collection"

# Check nested element
assert hdca["elements"][0]["object"]["elements"][0]["element_type"] == "hda"

# Check dataset state
assert element["object"]["state"] == "ok"

# Check tags
assert "group:type:single" in element["object"]["tags"]
```
